...
function Ninekufm(){
	...
	this.tip_space = "    ";
	this.console = { 
		nullline1:{color:"white", str:""},
		nullline2:{color:"white", str:""},
		fenlei:{color:"str", str: this.fenlei + "分类未加载"},
		fenge:{color:"str", str: this.fenge + "风格未加载"},
		splitline1:{color:"white", str:
			"----------------------------------------------------------------------------"},
		gqname:{color:"green", str:this.music_space + "歌曲信息未加载"},
		gsname:{color:"green", str:""},
		zjname:{color:"green", str:""},
		volume:{color:"green", str:""},
		time:{color:"green", str:""},
		nullline3:{color:"white", str:""},
		splitline2:{color:"white", str:
			"----------------------------------------------------------------------------"},
		nullline4:{color:"white", str:""},
		lrc1:{color:"white", str:""},
		lrc2:{color:"white", str:""},
		lrc3:{color:"green", str:""},
		lrc4:{color:"white", str:""},
		lrc5:{color:"white", str:""},
		nullline4:{color:"white", str:""},
		nullline5:{color:"white", str:""},
		tip:{color:"white", str:""},
		tip1:{color:"green",str:this.tip_space + "n:下一曲  space:播放(暂停)  <:减小音量  >:增加音量  q:退出 s:喜爱"},
		tip2:{color:"green",str:this.tip_space + "up,down(j,k):选择分类  left,right(h,l):选择风格  d:删除  t:讨厌"},
		debug:{color:"white", str:""},
	};
	this.help = {};
...
Ninekufm.prototype.action = function (){
	...
	/* 默认服务端动作处理函数 */
	var action_handle = function(error, response, body){
		if (response && response.statusCode == 200){
			var ret_str = JSON.parse(body);
			console.log(ret_str["str"]);
		}else{
	...
	/* 与云端交换数据格式
	 * post发送，json格式，action代表要执行的动作
	 * data代表用户的配置信息的base64;
	 * 接受格式，json格式，status表示返回状态
	 * (ok，表示希望做的事情已完成，error，表示出错)
	 * str，表示状态解释(一般用来显示错误信息)
	 * data，可选，一般在获得云端数据时用到
	 * (对源json字符串化后采用base64编码,
	 * 在download时表示云端存储的用户配置的data部分)
	 */
	if (action == undefined){
		...
		self.help["adduser"] = "adduser\t\t\t注册一个云端用户";
		self.help["config"] = "config\t\t\t配置本地用户，如果没有会新建，存在会只修改用户信息";
		self.help["init"] = "init\t\t\t初始化一个本地配置文件，如果存在会删除以前的";
		self.help["help"] = "help\t\t\t输出帮助文档";
		self.help["download"] = "download\t\t下载云端配置文件";
		self.help["upload"] = "upload\t\t\t上传本地配置文件";
		self.help["forget"] = "forget\t\t\t忘记密码找回(目前只提供这一种修改密码的方法)";
		...
		self.load_user_config("null");
		self.user_config["email"] = readlineSync.question("email:");
		self.user_config["password"] = readlineSync
			.question("new password:", {hideEchoBack: true, mask:''});
		var ok = readlineSync.question("Do you want to continue? (y/N)");
		if (ok != "y" && ok != "Y"){
			process.exit();
		}
		...
		self.load_user_config();
		self.user_config["data"] = "";
		action_handle = function(error, response, body){
			if (response && response.statusCode == 200){
				var ret_str = JSON.parse(body);
				if(ret_str["status"] == "ok"){
					self.user_config["data"] = JSON.parse(Buffer(ret_str["data"],"base64"));
					fs.writeFileSync(self.user_config_path,
						JSON.stringify(self.user_config, null, 4), "utf-8");
				}
				console.log(ret_str["str"]);
		...
		var server = http.createServer(function(request, response){
			...
			response.writeHead(200, {"Content-Type":"audio/mpeg", "Connection":"Keep-Alive"});
	...
	var buffer = Buffer(JSON.stringify(self.user_config, null, 4));
	var post_data = buffer.toString("base64");
	var remote = request.post( self.remote_addr, action_handle);
	var form = remote.form();
	form.append("action", action);
	form.append("data", post_data);
}
Ninekufm.prototype.init = function (){
	var self = this;
	if ( ! fs.existsSync(self.user_config_dir) )
		fs.mkdir( self.user_config_dir);
	if( self.action() ){
...
Ninekufm.prototype.settip= function (str, timeout){
	...
	self.setconsole("tip", {str:str, timeout:timeout});
}
...
Ninekufm.prototype.new_music_init= function (){
	...
	if (self.mode == "server"){
		...
		self.speaker.pipe(self.server_speaker, {end:false});
...
Ninekufm.prototype.start = function (){
	...
	if (! self.music_list.length){
		self.setconsole("tip", {str:"本风格内歌曲个数为0，将随机热门分类中一个风格", timeout:3000});
		setTimeout(function(){self.run_fenge();},50);
		return;
	}
	...
	if (select == "lrc"){
		...
		if(! option){
			...
			if (self.lrc_index >= self.geciobj.length){
				self.geci_ok = false;
				return;
			}else if(self.timer.time() < self.geciobj[self.lrc_index].time){
			}else{
		...
		if (option){
			...
			setTimeout(function(){self.setconsole("tip");}, option["timeout"]);
		}else{
